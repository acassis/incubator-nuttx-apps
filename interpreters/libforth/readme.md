...
# STM32F4 VERSION NOTES

This is the embedded version of libforth, most of this document will not apply,
the embedded version is a work in progress.

## EMBEDDED VERSION TODO

* Customize libforth for an embedded environment
  - Customize it specifically for the [STM32F4][] and running under NuttX.
* This embedded version of the interpreter will need synchronizing with the
main-line version.
* There are a number of ways this interpreter could be improved, mainly in
terms of memory usage, that would make this program more suitable for use
in an embedded context, this best done on the mainline branch.
* The build system needs to be modified so that a core file can be generated
from a hosted Forth, *core.gen.c* currently is a **magic** file that has
been compiled and added from an external source with no way to regenerate it.
* A lot of the code needs removing as it is no longer suitable / relevant for
an embedded system.
...
[STM32F4]: http://www.st.com/en/microcontrollers/stm32f4-series.html
